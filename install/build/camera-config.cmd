...
:: camera-config.cmd - Configures Axis IP cameras via axis-cgi & VAPIX API
...
:: Find camera
set upnpscan=c:\local\bin\upnpscan.exe

%upnpscan% -m | findstr LOCATION

FOR /f "tokens=3" %%I IN (
'\local\bin\upnpscan.exe -m ^| findstr "LOCATION"'
) DO SET ipAddress=%%I

REM "found 1"
::IF NOT x%ipAddress:10.130=%==x%ipAddress% (
::ECHO "good found 1" + %ipAddress%
::ECHO "do_something_else" )

REM "found 2"
::IF NOT x%ipAddress:10.140=%==x%ipAddress% (
::ECHO "crap found 2" + %ipAddress%
::ECHO "do_something_else" )
