...
#$PSDefaultParameterValues += @{'Invoke-WebRequest:ErrorAction' = "SilentlyContinue"}
...
# --------------------------------------------------------------------------------------------
Write-Host "${basename}: Setup D drive ... - Virtual Machine Deployments Only"

Write-Host "${basename}: Looking for formatted logical drives with actice NTFS partitions ..."
...
Write-Host "${basename}: Checking for Virtual Machine Deployment ..."
if ((Get-WmiObject -Class Win32_ComputerSystem).Model -eq "VMware Virtual Platform") {
    Write-Host "${basename}: Virtual Machine Deployment Detected ... Setting up required virtual logical drives ..."
    ...
    if (!(Test-Path -Path "D:\done.txt")) {
        Write-Host "${basename}: Preparing D drive"
...
Write-Host "${basename}: Currently DISABLED" -ForegroundColor Red
# # --------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------
Write-Host "${basename}: LockerLife Windows Configuration" -ForegroundColor Magenta
...
Get-ItemProperty -Path "HKLM:\SOFTWARE\Clients\StartMenuInternet"
## FIXME
...
w32tm.exe /config /manualpeerlist:"asia.pool.ntp.org", 0x8 /syncfromflags:MANUAL
# w32tm.exe /config /reliable:YES /syncfromflags:manual /manualpeerlist:"stdtime.gov.hk 0.asia.pool.ntp.org 3.asia.pool.ntp.org"
...
Write-Host "${basename}: Disable Welcome Logon Screen and Require CTRL-ALT-DEL"
...
reg.exe LOAD "hku\temp" "$env:USERPROFILE\..\Default User\NTUSER.DAT"
reg.exe ADD "HKEY_USERS\.DEFAULT\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" /v VisualFXSetting /t REG_DWORD /d 2 /f
...
reg.exe add "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v ForceClassicControlPanel /d 1 /f
# do not highlight newly installed programs
reg.exe add "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\Advanced" /v Start_NotifyNewApps /d 0 /f

# --------------------------------------------------------------------
Write-Host "${basename}: Setup Default User Registry"
$defaultUserHivePath = $env:SystemDrive + "\Users\Default\NTUSER.DAT"
$userLoadPath = "HKU\TempUser"

# Load Default User Registry Hive
reg.exe load $userLoadPath $defaultUserHivePath | Out-Host
# Create PSDrive
...
[gc]::collect()
# Unload Hive
reg.exe unload $userLoadPath | Out-Host
...
Write-Host "${basename}: Attempt to close extra open windows"
...
## vol down
Write-Host "${basename}: Set volume to Mute"
...
Write-Host "${basename}: Set lock screen background image"
...
reg.exe ADD "HKLM\SYSTEM\CurrentControlSet\Control" /v Start /t REG_DWORD /d 4 /f
Set-RegistryKey -Path "HKLM:\SYSTEM\CurrentControlSet\Control" -Name "Start" -Value 4

# --------------------------------------------------------------------
# Internet Explorer customizations ...
reg.exe ADD "HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Internet Explorer\Main" /v DisableFirstRunCustomize /t REG_DWORD /d 1 /f
reg.exe ADD "HKEY_CURRENT_USER\Software\Microsoft\Internet Explorer\Main" /v "Start Page" /t REG_SZ /d "http://lockerlife.hk/deploy" /f
...
reg.exe add "HKLM\SOFTWARE\Policies\Microsoft\Windows NT\CurrentVersion\NetworkList\Signatures\FirstNetwork" /v Category /t REG_DWORD /d 00000001 /f

Write-Host "${basename}: disable netbios"
C:\Windows\System32\Wbem\wmic.exe nicconfig where TcpipNetbiosOptions=0 call SetTcpipNetbios 2
C:\Windows\System32\Wbem\wmic.exe nicconfig where TcpipNetbiosOptions=1 call SetTcpipNetbios 2

## Windows Firewall
Write-Host "${basename}: Configure Windows Firewall"
Write-Host "${basename}: turn on firewall"
...
if (!(Test-Path "c:\local\lib\wasp.dll" -ErrorAction SilentlyContinue)) {
...
Write-Host "${basename}: DISABLE GUEST USER"
...
if (Get-CimInstance win32_useraccount | where { $_.Name -eq "AAICON" }) {
    ...
    net.exe user AAICON Locision123 /add
    net.exe user AAICON Locision123 /expires:never /passwordchg:no
...
if (!(Get-CimInstance win32_useraccount | where { $_.Name -eq "taskbot" })) {
    ...
    psexec.exe -accepteula -nobanner -u taskbot -p TaskBotPassW0rd cmd /c dir
}
...
dism.exe /online /disable-feature /featurename:InboxGames /NoRestart
dism.exe /online /disable-feature /featurename:FaxServicesClientPackage /NoRestart
dism.exe /online /disable-feature /featurename:WindowsGadgetPlatform /NoRestart
dism.exe /online /disable-feature /featurename:OpticalMediaDisc /NoRestart
...
# --------------------------------------------------------------------
Write-Host "${basename}: Next stage ... 00-bootstrap" -ForegroundColor Magenta
